...
# GLOBAL SETTINGS
# some parameters are borrowed from https://github.com/rrmina/fast-neural-style-pytorch 
# some are based on the paper
...
BATCH_SIZE = 8
CONTENT_WEIGHT = 1e5 # 17
STYLE_WEIGHT = 1e10 # 25
ADAM_LR = 0.001
...
SEED = 35
PLOT_LOSS = 1
TEST_INPUT = "images/test_1.jpg"
TEST_MODEL = "models\epoch_2_Tue_Dec__5_22-45-29_2023_100000.0_10000000000.0.model"
...
def train():
    ...
    for e in range(NUM_EPOCHS):
        ...
        for batch_id, (x, _) in enumerate(train_loader):
            ...
            # According to paper using relu3_3 for content loss
            # has less distortions than higher level features
            # But according to our experiments, using relu2_2 can 
            # generate good results with only one epoch
...
def test_stylize():
    ...
    content_image = utils.load_image(TEST_INPUT)
    ...
    with torch.no_grad():
        ...
        state_dict = torch.load(TEST_MODEL)
        style_model.load_state_dict(state_dict)
        style_model.to(device)
        style_model.eval()
        output = style_model(content_image).cpu()
    utils.save_image(SAVE_IMAGE_PATH+"test_out.jpg", output[0])
# train()
test_stylize()